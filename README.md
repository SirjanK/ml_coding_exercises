...
I initially trained models with learned absolute encodings (simple matrix $\Pi\in \mathbb{R}^{T, E}$ where `T` is seq length, `E` is embedding size), but once your KV cache hits `T` entries,
